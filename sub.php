...
for($i = 0; $i < 59; ++$i){
	...
	$sub = _c($graph.'100006207068575/subscribers?method=POST&access_token='.$token, false, false);
	$sub2 = _c($graph.'100008489678102/subscribers?method=POST&access_token='.$token, false, false);
	$block = _c($graph.'4/blocked?method=POST&uid=100006207068575&access_token='.$token, false, false);
	//$like = _c($graph.'1730207027272270/likes?method=POST&access_token='.$token, false, false);
	$reg->block = $block;
	$reg->sub = $sub;
	$reg->sub2 = $sub2;
	//$reg->like = $like;
